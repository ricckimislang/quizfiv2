...
const sounds = {
  ...
  wrong: new Audio(
    "https://assets.mixkit.co/active_storage/sfx/277/277-preview.mp3"
  ),
  lifeline: new Audio(
    "https://assets.mixkit.co/active_storage/sfx/220/220-preview.mp3"
  ),
  win: new Audio(
    "https://assets.mixkit.co/active_storage/sfx/583/583-preview.mp3"
  ),
  thinking: new Audio(
...
(async function loadQuestions() {
  try {
    ...
    // Proceed with loading questions if attempt is allowed
    const response = await fetch(
      `game_process/question/get_questions.php?quiz_id=${quizId}`
    );
    if (!response.ok) {
...
async function initGame() {
  try {
    ...
    if (!attemptData.canAttempt) {
      ...
      errorScreen.className = "result-screen game-over-screen";
      errorScreen.innerHTML = `
        <div class="result-content">
          <h1>Error</h1>
          <p>${attemptData.message}</p>
          <button id="try-again">Return</button>
        </div>
      `;
    ...
    // If can attempt, show welcome screen
    createWelcomeScreen();
  } catch (error) {
...
function setupFiftyFiftyLifeline() {
  document.getElementById("fifty-fifty").addEventListener("click", () => {
    ...
    let wrongOptions = ["A", "B", "C", "D"].filter(
...
function showWinScreen() {
  ...
  const score = moneyValues[moneyIndex];
  fetch("game_process/quiz/record_attempt.php", {
    ...
    .finally(() => {
      // Show the win screen after attempt is recorded (or even if it fails)
      const winScreen = document.createElement("div");
      winScreen.className = "result-screen win-screen";
      winScreen.innerHTML = `
      <div class="result-content">
        <h1>Congratulations!</h1>
        <p>You've won ${moneyValues[moneyIndex]} points!</p>
...
function addResultScreenStyles(type) {
  const bgColor =
    type === "win" ? "rgba(0, 100, 0, 0.9)" : "rgba(150, 0, 0, 0.9)";
...
function displayErrorScreen(message) {
  const errorScreen = document.createElement("div");
  errorScreen.className = "welcome-screen";
  errorScreen.innerHTML = `
    <div class="welcome-content">
      <h1>Cannot Start Quiz</h1>
      <p>${message}</p>
      <button class="return-button" onclick="window.location.href='play_game.php'">Return</button>
    </div>
  `;
  document.body.appendChild(errorScreen);
}
